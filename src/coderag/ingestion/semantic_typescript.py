...
def _parse_import_bindings(
    ...
    *,
    ...
    """Parse TS imports into local bindings that point to exported symbols."""
    ...
    for line in file_obj.content.splitlines():
        ...
        import_clause = import_clause.replace("import type", "", 1)
...
def _build_top_level_file_import_relations(
    ...
    *,
    ...
    """Emit TS file-scoped import edges from top-level import lines."""
...
def extract_typescript_semantic_relations(
    ...
    scanned_paths = {item.path for item in _typescript_files(scanned_files)}
    relations: list[SemanticRelation] = []
